...
* There is a full screen background image shown when the user visits the site and images are shown for each of the areas and attractions which can be clicked and viewed in a gallery.
* A search box allows users to search for a place which is then shown on the map. The map also has pre-determined markers that match the areas and attractions shown on the site. 
* The user can send a message through the contact form if they had any questions.

### User Interaction:

* When first opening the site the user can see a full screen background image and a callout that explains the use of the site. 
...
It also tells them the markers on the map relate to the places mentioned below the map and that they can also search using the search box.
...
The user will know if their message is sent correctly as an alert will appear. An alert will also appear if the message does not send and advises them to try again. When the user clicks "OK" on these alerts the page is refreshed.
...
* Sometimes when opening the site in developer tools and it is set to responsive, whitespace can be seen running from top to bottom on the right-hand side.
    ...
    * It does not happen when viewing with a physical mobile device.
...
* Change screen size to mobile and tablet under 991px width to see the menu turn into a collapsible to a burger menu.
...
* Verify the map and search box fit different screen sizes well.
...
* Verify the form submits by filling out the inputs and pressing the button.
